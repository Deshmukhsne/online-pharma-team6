...
const AdminDashboard = () => {
    ...
    const [drugCount, setDrugCount] = useState(0); // State for drug count
    const [memberCount, setMemberCount] = useState(0);
    const [loading, setLoading] = useState(true);
    useEffect(() => {
        const fetchDrugCount = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/drugs/count');
    ...
    useEffect(() => {
        const fetchMemberCount = async () => {
            try {
                ...
                console.log('Member count response:', response.data); // Debug log
    ...
    const stats = [
        { label: "Total Users", value: memberCount, icon: <FaUsers style={{ color: '#1abc9c' }} /> },
        { label: "Total Drugs", value: drugCount, icon: <FaPills style={{ color: '#3498db' }} /> },
